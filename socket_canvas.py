...
def handle_conn(conn):
  ...
  def process_commands():
    while queue.qsize():
      ...
      if command == 'rect':
        try:
          x, y, width, height, color = args.split(',')
          x = int(x) + LEFT_PAD
          y = int(y) + TOP_PAD
          width = int(width)
          height = int(height)
          ...
          print 'Bad rect command', e
        continue

      if command == 'text':
        try:
          x, _, args = args.partition(',')
          y, _, args = args.partition(',')
          color, _, text = args.partition(',')
          x = int(x) + LEFT_PAD
          y = int(y) + TOP_PAD
          w.create_text(x, y, text=text, anchor=NW, fill=color, font='Courier')
        except Exception as e:
          print 'Bad text command', e
        continue
